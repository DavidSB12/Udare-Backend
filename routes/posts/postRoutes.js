...
router.post('/addOld', ImageUpload.fields([{ name: 'image', maxCount: 1 }, { name: 'post', maxCount: 1 }]),addPost);
router.put('/:id', updatePost);
router.delete('/:id', deletePost);
router.post('/:id/upload', uploadImage);
router.post('/:id/addComment', addComment)
router.post("/add", upload.fields([{ name: 'image', maxCount: 1 }, { name: 'post', maxCount: 1 }]),newAddPost);
